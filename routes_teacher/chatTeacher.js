...
router.get("/chatTeacher", function(req, res) {
    if (req.session.uid) {
        if (req.session.role == "teacher") {
            signupModel.find({ registred_id: { $ne: req.session.uid } }, function(err, data) {
                res.render("chatTeacher", { userInfo: req.session.name, role: req.session.role, userId: req.session.uid, chatStudents: data, pic: req.session.pic });
            });
        } else {
